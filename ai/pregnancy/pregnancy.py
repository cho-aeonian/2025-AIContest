...
model = YOLO('pregnancy.pt')
cap = cv2.VideoCapture("pregnancy.mp4")

fps = cap.get(cv2.CAP_PROP_FPS)
frame_delay = int(1000 / fps) if fps > 0 else 200

root = tk.Tk()
root.title("Pregnancy GUI")
...
box_count_label = tk.Label(root, text="pregnancy count:", font=("Arial", 10))
...
def capture_frame():
    ...
    box_count_label.config(text=f"pregnancy count: {num_boxes}")
